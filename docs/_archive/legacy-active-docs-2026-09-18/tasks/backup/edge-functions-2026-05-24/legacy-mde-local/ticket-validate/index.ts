...
function isHumanReasonCode(value: unknown): value is HumanReasonCode {
  return typeof value === "string" && value in HUMAN_REASONS;
}
...
Deno.serve(async (req) => {
  ...
  const rawRpcData = rpcDataRaw as Partial<TicketValidateRpcData> | null;
  const rpcData: TicketValidateRpcData = {
    result: isHumanReasonCode(rawRpcData?.result) ? rawRpcData.result : "unknown_token",
    details: rawRpcData?.details ?? {
      unexpected_result: rawRpcData?.result ?? null,
    },
  };
