...
const RequestSchema = z.object({
  ...
  /** Default 24h, max 168h (7 days). Out-of-range → 400. */ ttl_hours: z
    .number().int().min(1).max(168).optional(),
  /** When true, bumps staff_link_version (revokes outstanding tokens) and returns the new version. */ revoke:
    z.boolean().optional(),
});
Deno.serve(async (req) => {
  ...
  const { data: event, error: eventErr } = await userClient.from("events")
    .select("id, organizer_id, staff_link_version").eq("id", body.event_id)
    .maybeSingle();
  if (eventErr) {
  ...
  if (body.revoke === true) {
    ...
    const { data: bumped, error: bumpErr } = await service.rpc(
  ...
  return jsonResponse(
    {
      ...
      data: {
        staff_link_url:
          `${APP_BASE_URL}/staff/check-in/${event.id}?token=${jwt}`,
        jwt,
