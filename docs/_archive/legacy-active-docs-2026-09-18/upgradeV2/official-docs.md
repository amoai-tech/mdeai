...
| `useCoAgent` | `useAgent` |
| `useCopilotChat` | `useAgent` + stock `CopilotChat` from `/v2` with slot overrides (`assistantMessage`, `markdownRenderer`). **`useCopilotChatHeadless_c` is v1-only** — not exported from `@copilotkit/react-core/v2` on pinned 1.55.2; do not mix v1 headless + v2 subpath. Optional: `useCopilotChatConfiguration`. |
