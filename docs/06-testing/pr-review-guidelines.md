...
- `required_approving_review_count` is **1**. Branch protection lives outside the repository, so it is applied through the GitHub API, not by a file here. What it actually enforces was **measured, not assumed** (PR #124): the rule is binding — the pull request showed `reviewDecision: REVIEW_REQUIRED` before any review arrived — but **a bot approval satisfies it**. `sourcery-ai[bot]` submitted the `APPROVED` review that decided the merge, and the only human collaborator is the author, who cannot approve their own pull request. So this does **not** guarantee that a human read the change. Two settings look like they would strengthen it and do not, while this repository has a single human collaborator: disabling Sourcery's approvals removes the only approval an ordinary merge can obtain, leaving admin bypass as the sole route (`enforce_admins: false`, so the admin can still bypass — the gate degrades rather than blocking outright); and `require_code_owner_reviews` is a **no-op while no `CODEOWNERS` file exists**, which is the case here, so it only bites once a rule exists whose owner cannot approve. The only configuration that produces a genuine human review gate is a **second human collaborator**.
