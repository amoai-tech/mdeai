...
**Canonical owner folders use their exact `.claude/skills/` names.** Primary runtime/domain owners are `copilotkit`, `mastra`, `supabase`, `gemini`, `maps`, `stripe`, `nextjs`, `cloudinary`, `events`, and `real-estate`; task/ops owners include `tasks`, `task-verifier`, `testing`, and `code-review`. `nextjs` also owns Vercel application-platform work through progressive-disclosure references. Lifecycle/worktree/PR execution is owned by `tasks`. See Section A for the complete current inventory.

`real-estate` is the single canonical owner; retired real-estate aliases are not part of the active skill graph.
...
## B — `.agents/skills` compatibility links (22)
...
Current compatibility names: `_template`, `cloudinary`, `code-review`, `copilotkit`, `events`, `gemini`, `maps`, `mastra`, `mermaid-diagrams`, `nextjs`, `playwright-cli`, `real-estate`, `research`, `stripe`, `supabase`, `systematic-debugging`, `task-verifier`, `tasks`, `testing`, `using-mde-skills`, `wireframe`, `writing-skills`.
...
| `.agents/skills` compatibility symlinks | `22` |
| Broken `.agents/skills` links | `0` |
| Noncanonical `.agents` targets | `0` |
| `.agents-only` skills | `0` |
| Real-estate canonical paths | only `real-estate` remains active |
